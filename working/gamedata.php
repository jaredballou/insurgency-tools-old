...
$gamedata_root="${rootpath}/thirdparty/sourcemod/gamedata";
$gamedata_root="/home/insserver/serverfiles/insurgency/addons/sourcemod/gamedata";
function MergePath() {
	$args = array_filter(func_get_args());
	$abspath = ($args[0][0] == "/") ? "/" : "";
	$args = preg_replace(array('|^/|','|/$|'),'', $args);
	$merged = $abspath.implode("/",$args);
	$merged = preg_replace('|//|','/', $merged);
//	var_dump($merged);
	return $merged;
}
$gamedata_files = array();
$gamedata_data = array("Games" => array());
...
//ProcessGameDataDir("${gamedata_root}/insurgency");
function DumpArray($gamedata_data,$levels=2){
	$levels--;
	$result = array();
	foreach ($gamedata_data as $key => $val) {
		$result[$key] = (($levels) && is_array($val)) ? DumpArray($val,$levels) : $key;
	}
	return $result;
}
var_dump(parseKeyValues(file_get_contents(MergePath($gamedata_root,"insurgency.games.txt"))));
//var_dump(DumpArray($gamedata_data,3));
exit;
?>
